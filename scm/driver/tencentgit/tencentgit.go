...
// Package tencentgit implements a GitLab client.
package tencentgit
...
// default git.code.tencent.com address.
func NewDefault() *scm.Client {
	client, _ := New("https://git.code.tencent.com")
	return client
}
...
type tencentgitNamespace struct {
	ID                          int    `json:"id"`
...
// time format not in RFC 3339, like "2006-01-02T15:04:05+0000"
...
func (c *wrapper) findNamespaceByName(ctx context.Context, name string) (*tencentgitNamespace, error) {
	in := url.Values{}
	in.Set("search", name)
	path := fmt.Sprintf("api/v3/namespaces?%s", in.Encode())

	var out []*tencentgitNamespace
